...
class KWSDataModule(LightningDataModule):
    def __init__(self, path, batch_size=128, num_workers=0, n_fft=512, 
                 n_mels=128, win_length=None, hop_length=256, patch_num=8,
                 class_dict={}, **kwargs):
...
def get_args():
    ...
    parser.add_argument('--depth', type=int, default=20, help='depth')
...
args = get_args()
path = os.path.join(path, "data", "speech_commands")
...
datamodule = KWSDataModule(batch_size=args.batch_size, num_workers=args.num_workers,
                               path=path, n_fft=args.n_fft, n_mels=args.n_mels,
